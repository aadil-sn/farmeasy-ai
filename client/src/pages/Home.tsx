...
function Market({ role, lots, requirements, onDetail, onOffer, onEdit, onCreateLot, onCreateRequirement }: { role: MarketplaceRole; lots: CropLot[]; requirements: Requirement[]; onDetail: (lot: CropLot) => void; onOffer: (lot: CropLot) => void; onEdit: (lot: CropLot) => void; onCreateLot: () => void; onCreateRequirement: () => void }) {
  ...
  return <>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filtered.map(lot => <LotCard key={lot.id} lot={lot} editable={isFarmer && lot.farmer === "Kamil"} onDetail={onDetail} onAction={isFarmer && lot.farmer === "Kamil" ? onEdit : onOffer} />)}</div>
  </>;
}
...
function LotForm({ initial, onClose, onSave, onRemove }: { initial?: CropLot | null; onClose: () => void; onSave: (lot: CropLot) => void; onRemove: (lot: CropLot) => void }) {
  ...
  function save(event: React.FormEvent) { event.preventDefault(); if (!form.product || !Number(form.quantity) || !Number(form.price)) { toast.error("Add a crop, quantity, and minimum price first."); return; } onSave({ id: initial?.id || `LOT-${Math.floor(80 + Math.random() * 19)}`, product: form.product, category: "Vegetables", quantity: Number(form.quantity), unit: "kg", farmer: "Kamil", village: form.village, harvestDate: form.harvest, quality: form.quality, availability: form.availability as CropLot["availability"], agreedPrice: Number(form.price), logistics: 2.2, handling: .5, fee: .7, distance: 42, verified: true }); }
...
export default function Home() {
  ...
  function decideOffer(offer: BuyerOffer, decision: "Accepted" | "Rejected") { setOffers(current => current.map(item => item.id === offer.id ? { ...item, status: decision } : item)); if (decision === "Accepted") setLots(current => current.map(item => item.id === offer.lotId ? { ...item, availability: "Reserved" } : item)); recordActivity("farmer", decision === "Accepted" ? "offer_accepted" : "offer_rejected", offer.id, `Kamil ${decision.toLowerCase()} ${offer.id}.`); addNotification({ role: "buyer", title: `${offer.id} ${decision.toLowerCase()}`, body: decision === "Accepted" ? "The crop lot is reserved and order coordination can begin." : "The farmer declined this offer. Browse comparable verified lots.", tone: decision === "Accepted" ? "mint" : "pink" }); toast.success(`Offer ${offer.id} ${decision.toLowerCase()}.`); }
